...
struct Offset
{
    ...
    void setValues(int ID, int num_procs, int number_of_graphs)
    {
        ...
        int block_end = std::min(block_num*block_size, number_of_comparisons);
        offset.i_start = std::floor(((2*number_of_graphs-1) - std::sqrt(9-4*number_of_graphs*(1-number_of_graphs)-8*block_start))/2);
        offset.i_end =   std::floor(((2*number_of_graphs-1) - std::sqrt(9-4*number_of_graphs*(1-number_of_graphs)-8*block_end))/2);
        offset.j_start = block_start + (offset.i_start*(offset.i_start+3-2*number_of_graphs))/2;
        offset.j_end = block_end + (offset.i_end*(offset.i_end+3-2*number_of_graphs))/2;
    }
			...
			{	
				...
				try
				{
					...
					{	
						...
						std::cout << "Process " << ID << ": " << i << " " << j << std::endl;
						result.frob_norm = 1;
