...
struct IsoRank_Result isoRank(DenseMatrix<DT>& matrix_A, DenseMatrix<DT>& matrix_B)
{

    // Degree distribution statistics
    
    SymDenseMatrix<DT> kron_prod = matrix_A.kron(matrix_B);
    ...
    for(int i=0; i < kron_prod.getSize(); i++ )
    {
        ...
        std::cout << L << std::endl;
         ...
         DenseMatrix<DT>* lTimesD = L->vec_times_mat(D_neg0pt5,L->getNumberOfRows());
         DenseMatrix<DT>* Ms = lTimesD->mat_times_vec(D_neg0pt5, L->getNumberOfRows());
         delete lTimesD;
         if(!Ms->isSymmetric())
         {
     ...
     for(int k=0;k<kron_prod->getNumberOfColumns();k++) {
         ...
         if(eigenvector!=NULL) {
             ...
             DenseMatrix<double>* scores_copy=new DenseMatrix<double>(*scores);
